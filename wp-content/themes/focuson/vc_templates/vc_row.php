<?php
	...
	$output .= '<div '.$id.' class="nz-section '.sanitize_html_class($animation_dir).' disable-'.sanitize_html_class($disable_element).' autoheight-'.sanitize_html_class($autoheight).' animate-'.sanitize_html_class($img_animate).' full-width-'.sanitize_html_class($full_width).' '.sanitize_html_class($el_class).'" '.$data_img_width.' '.$data_img_height.' data-animation-speed="'.absint($animation_speed).'" data-parallax="'.esc_attr($parallax).'" style="'.esc_attr($styles).'">';
