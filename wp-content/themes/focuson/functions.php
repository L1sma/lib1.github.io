...
/*======================*/
    ...
    if (class_exists('OCDI_Plugin')) {
        ...
        function focuson_ninzio_intro_text( $default_text ) {
            $default_text = '<br><br><div class="ocdi__intro-text">
                <p class="about-description">
                    '.esc_html__( "Importing demo data (post, pages, images, theme settings, ...) is the easiest way to setup your theme.", "focuson" ).'
                    '.esc_html__( "It will allow you to quickly edit everything instead of creating content from scratch.", "focuson" ).'
                </p>
                <hr>
                <p>'.esc_html__( "When you import the data, the following things might happen:", "focuson" ).'</p>
                <ul>
                    <li>'.esc_html__( "No existing posts, pages, categories, images, custom post types or any other data will be deleted or modified.", "focuson" ).'</li>
                    <li>'.esc_html__( "Posts, pages, images, widgets, menus and other theme settings will get imported.", "focuson" ).'</li>
                    <li>'.esc_html__( "Please click on the Import button only once and wait, it can take a couple of minutes.", "focuson" ).'</li>
                    <li>'.esc_html__( "If you want to change the homepage version after import, do not import another demo, go to WordPress settings >> Reading and choose different homepage version as your front-page.", "focuson" ).'</li>
                    <li>'.esc_html__( "If you want to import pages/posts/custom post type/menu etc. separately use regular WordPress importer", "focuson" ).'</li>
                    <li>'.esc_html__( "After import don't forget to import Revolution Slider separately from Revolution Slider settings page", "focuson" ).'</li>
                </ul>
                <hr>
            </div><div class="ocdi__intro-text">'.esc_html__("Before importing, please check", "focuson").' <a target="_blank" href="http://ninzio.com/knowledgebase/demo-data-importexport/">'.esc_html__("Demo import guide", "focuson").'</a></div><br><br>';
            return $default_text;
        }
        add_filter( 'pt-ocdi/plugin_intro_text', 'focuson_ninzio_intro_text' );

        function focuson_ninzio_config_after_import_files() {

            $header_menu      = get_term_by('name', 'Header Menu', 'nav_menu');
            $one_page_bullets = get_term_by('name', 'One Page Bullets', 'nav_menu');
        ...
        add_action( 'pt-ocdi/after_all_import_execution', 'focuson_ninzio_config_after_import_files', 10, 2 ); 
    }
...
/*======================*/
    ...
    /*-------------------*/
        ...
        if (class_exists('Woocommerce')){
            ...
            $products_per_page = (isset($GLOBALS['focuson_ninzio']['products-per-page']) && !empty($GLOBALS['focuson_ninzio']['products-per-page'])) ? $GLOBALS['focuson_ninzio']['products-per-page'] : get_option( 'posts_per_page' );
            ...
            add_action( 'woocommerce_before_add_to_cart_form',  'focuson_ninzio_social_share', 2);
            ...
            function focuson_ninzio_social_share(){ ?>
                <?php global $post; ?>
                <div class="post-social-share nz-clearfix">
                    ...
                    <div class="social-links left nz-clearfix">
                        ...
                        <a title="<?php echo esc_html__("Share on Pinterest", 'focuson'); ?>" class="post-pinterest-share icon-pinterest" target="_blank" href="http://pinterest.com/pin/create/button/?url=<?php the_permalink(); ?>&media=<?php $url = wp_get_attachment_url( get_post_thumbnail_id($post->ID) ); echo esc_url($url); ?>"></a>
            ...
            function focuson_ninzio_clearfix_div(){ ?>
                </div>
...
/*======================*/
    ...
    function focuson_ninzio_script()
    {
        ...
        wp_enqueue_script( 'masonry', FOCUSON_NINZIO_TEMPPATH . '/js/masonry.pkgd.min.js', array('jquery'), '', true);
