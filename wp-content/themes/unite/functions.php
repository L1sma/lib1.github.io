...
if ( ! function_exists( 'spekt_cp' ) ) {
 
// Опишем требуемый функционал
    function spekt_cp() {
 
        $labels = array(
            'name'                => _x( 'Спектакли', 'Post Type General Name', 'spekt' ),
            'singular_name'       => _x( 'Спектакли', 'Post Type Singular Name', 'spekt' ),
            'menu_name'           => __( 'Спектакли', 'spekt' ),
            'parent_item_colon'   => __( 'Родительский:', 'spekt' ),
            'all_items'           => __( 'Все записи', 'spekt' ),
            'view_item'           => __( 'Просмотреть', 'spekt' ),
            'add_new_item'        => __( 'Добавить новую запись в спектакли', 'spekt' ),
            'add_new'             => __( 'Добавить новую', 'spekt' ),
            'edit_item'           => __( 'Редактировать запись', 'spekt' ),
            'update_item'         => __( 'Обновить запись', 'spekt' ),
            'search_items'        => __( 'Найти запись', 'spekt' ),
            'not_found'           => __( 'Не найдено', 'spekt' ),
            'not_found_in_trash'  => __( 'Не найдено в корзине', 'spekt' ),
        );
        $args = array(
            'labels'              => $labels,
            'supports'            => array( 'title', 'editor', 'excerpt', ),
            'taxonomies'          => array( 'spekt_tax' ), 
            'public'              => true,
            'menu_position'       => 5,
            'menu_icon'           => 'dashicons-book',
        );
        register_post_type( 'spekt', $args );
 
    }
 
    add_action( 'init', 'spekt_cp', 0 ); 
 
}


function spektl_meta_box() {  
    add_meta_box(  
        'spektl_meta_box', // Идентификатор(id)
        'Спектакли - дополнительная информация', // Заголовок области с мета-полями(title)
        'show_my_red_metabox', // Вызов(callback)
        'spekt', // Где будет отображаться наше поле, в нашем случае в разделе Спектакли
        'normal',
        'high');
}  
add_action('add_meta_boxes', 'spektl_meta_box'); // Запускаем функцию
...
function category_shortcode( $atts, $content = null )
{
...
$lastposts = get_posts( 'numberposts='.$limit.'&order=DESC&orderby=rand&post_type=spekt');
...
add_shortcode('cat', 'category_shortcode');